...
void thingspeak_process(){
    ...
    if(sm_mqtt_client_get_state(p_app->mqtt_client) == MQTT_ST_CONNECTED){
        if(counter > p_app->thingspeak.period*100){
...
static void command_data_handle(THING_SPEAK_APP* p_thinkspeak){
    ...
    if(max_temp == NULL)
        return;
    cJSON* max_humi = cJSON_GetObjectItem(root, "max_humi");
    if(max_humi == NULL)
        return;
    ...
    p_thinkspeak->max_humi = (float)cJSON_GetNumberValue(max_humi);
    p_thinkspeak->period = (int)cJSON_GetNumberValue(period);
    cJSON_Delete(root);
}
